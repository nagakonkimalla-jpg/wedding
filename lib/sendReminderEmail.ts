...
function generateReminderEmailHTML(
  data: ReminderGuestData,
  eventData: ReminderEventData[]
): string {
  const logoUrl =
    "https://wedding-five-self-14.vercel.app/assets/images/pelli/logo.jpeg";
  const primaryColor = "#B8860B";

  const eventRows = eventData
    .map(({ event, adults, kids }) => {
      const guestLine =
        kids > 0
          ? `Adults: <strong>${adults}</strong> &nbsp;&middot;&nbsp; Kids: <strong>${kids}</strong>`
    ...
    <tr><td style="padding:12px 0;border-bottom:1px solid #f0ebe3;">
      ...
      <tr>
        ...
        <td style="vertical-align:top;">
          ...
          <p style="margin:3px 0 0;font-size:13px;color:#6B5B4E;font-family:Arial,sans-serif;">&#128101; ${guestLine}</p>
          ${event.dressCode ? `<p style="margin:3px 0 0;font-size:12px;color:#8B7355;font-family:Arial,sans-serif;">&#128087; ${event.dressCode}</p>` : ""}
          <p style="margin:6px 0 0;"><a href="${event.googleMapsUrl}" target="_blank" style="font-size:12px;color:${event.theme.primary};text-decoration:none;font-family:Arial,sans-serif;">View on Maps &rarr;</a></p>
        </td>
      </tr>
      </table>
    </td></tr>`;
...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width:560px;margin:0 auto;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.06);">
  ...
  <tr><td style="text-align:center;padding:16px 24px 4px;">
    <h1 style="margin:0;font-size:24px;color:${primaryColor};font-family:Georgia,serif;">One Week to Go!</h1>
  ...
  <tr><td style="padding:0 24px 20px;">
    <p style="margin:0;font-size:16px;color:#3D2B1F;font-family:Georgia,serif;text-align:center;line-height:1.6;">
      Dear <strong>${data.fullName}</strong>,
    </p>
    <p style="margin:12px 0 0;font-size:15px;color:#6B5B4E;font-family:Arial,sans-serif;text-align:center;line-height:1.6;">
      We're just <strong>one week away</strong> from the celebrations! Here's a reminder of the events you've RSVP'd to. We can't wait to see you there!
    </p>
  </td></tr>
...
export async function sendReminderEmail(
  ...
  if (!user || !pass) {
    console.warn(
      "GMAIL_USER or GMAIL_APP_PASSWORD not set, skipping reminder email"
    );
    return;
  }
  ...
  const eventNames = eventData.map((e) => e.event.title).join(", ");
  const subject = `One Week Reminder — ${eventNames} | Neelu & Aditya's Wedding`;

  await transporter.sendMail({
    from: `"Neelu & Aditya's Wedding" <${user}>`,
    to: data.email,
    subject,
    html: generateReminderEmailHTML(data, eventData),
  });
}
